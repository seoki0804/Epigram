    ...
    <div className="p-8">
      ...
      <section className="mb-8">
        ...
        <div className="space-x-4">
          ...
          <button
            className="hover:bg-red-600 h-10 w-36 rounded bg-red-500 text-white"
            onClick={logout}
          >
            로그아웃
          </button>
      ...
      <section className="mb-8">
        ...
        <div className="space-x-4">
          <button
            className="h-10 w-48 rounded bg-green-500 text-white hover:bg-green-600"
            onClick={() => handleAPICall<[]>(getEpigrams)}
          >
            에피그램 목록 가져오기
          </button>
          <button
            className="h-10 w-48 rounded bg-yellow-500 text-white hover:bg-yellow-600"
            onClick={() =>
          ...
          <button
            className="h-10 w-48 rounded bg-pink-500 text-white hover:bg-pink-600"
      ...
      <section className="mb-8">
        ...
        <div className="space-x-4">
        <button
  className="h-10 w-48 rounded bg-purple-500 text-white hover:bg-purple-600"
  onClick={() => handleAPICall<[string]>(getComments, ["1"])} // 예시 ID "1"
>
  댓글 가져오기
</button>
          <button
            className="h-10 w-48 rounded bg-indigo-500 text-white hover:bg-indigo-600"
            onClick={() =>
          ...
          <button 
            className="h-10 w-48 rounded bg-red-500 text-white hover:bg-red-600"
      ...
      {/* 결과값 */}
      <section>
        <h2 className="mb-4 text-xl font-semibold">API 응답</h2>
        {response && (
          <pre className="overflow-auto rounded-md bg-gray-100 p-4 text-sm text-black">
            {response}
          </pre>
        )}
        {error && <p className="text-red-500 font-semibold">❌ 오류: {error}</p>}
      </section>
    </div>
  );
};
